    ...
    if convert_from_caffe2:
        ...
        if inflation:
            ...
            for k, v in pre_train_dict.items():
                if k.endswith(".rel_pos_t"):
                    if v.size() != model_dict[k].size():
                        assert v.size(1) == model_dict[k].size(1)
                        new_rel_pos_t = torch.zeros_like(model_dict[k])
                        new_rel_pos_t[: v.size(0)] = v

                        new_rel_pos_t[v.size(0) :] = new_rel_pos_t[
                            v.size(0) - 1 : v.size(0)
                        ]

                        pre_train_dict[k] = new_rel_pos_t
                        logger.info(
                            f"Extending {k} from {v.size()} to {new_rel_pos_t.size()}"
                        )

            # for k in pre_train_dict.keys():
            #     if (
            #         "rel_pos" in k and (k.endswith("_h") or k.endswith("_w"))
