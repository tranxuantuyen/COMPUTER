...
def load_object_boxes(cfg, mode):
    """
    ...
    ann_filenames = cfg.COMPUTER.OBJECT
    detect_thresh = cfg.COMPUTER.OBJECT_SCORE_THRESH
    all_boxes = {}
    count = 0
    unique_box_count = 0
    with open(ann_filenames, "r") as f:
        for line in f:
            ...
            if score < detect_thresh or frame_sec not in AVA_VALID_FRAMES or label not in [1, 13, 18, 24, 25, 26, 27, 29, 30, 33, 34,  35, 36, 39, 40, 42, 43 , 44, 45, 46, 73] :
                continue
    ...
    logger.info(
        "Finished loading OBJECT annotations from: %s" % "".join(ann_filenames)
    )
    logger.info("Detection threshold: {}".format(detect_thresh))
    logger.info("Number of OBJECT boxes: %d" % unique_box_count)
    logger.info("Number of OBJECT annotations: %d" % count)
...
def load_boxes_and_labels(cfg, mode):
    ...
    logger.info(
        "Finished loading annotations from: %s" % ann_filenames
    )
    logger.info("Detection threshold: {}".format(detect_thresh))
