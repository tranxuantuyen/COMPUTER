...
for cur_iter, (inputs, labels,_, _,  meta) in tqdm(enumerate(train_loader)):
    ...
    pathlib.Path(args.temp_path).mkdir(exist_ok=True)
    ...
    feature[video_id][sec]['context_one_feature'] = encoder_feature[0].mean(dim=0).cpu().detach().numpy()

    # print(cur_iter)
    if cur_iter > 10:
        break
filename = args.temp_path + '/{}.h5'.format(args.mode)
